...
public class Follower extends JFrame {
	...
	private void initialize(String currentUser) {
	    ...
	    Follower.this.getContentPane().add(topPanel);
	    ...
	    // 텍스트 라벨 생성
	    JLabel textLabel = new JLabel("Follower");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                SwingUtilities.invokeLater(() -> {
                    try {
                        ...
                        Follower.this.dispose(); // 현재 프레임 닫기 (필요 시 유지)
                    } catch (Exception ex) {
                        ex.printStackTrace();
                    }
                });
	    ...
	    Follower.this.getContentPane().add(scrollPane);

	    // 모든 포스트 가져오기
	    java.util.List<String[]> allFollower = loadFollowers(currentUser);

	    // 각 follower 정보를 표시할 패널 생성 및 추가
	    int yOffset = 10; // 첫 follower 간격
	    int panelHeight = 70; // 각 패널의 높이
	    for (String[] follower : allFollower) {
	        JPanel followerPanel = new JPanel();
	        followerPanel.setLayout(null);
	        followerPanel.setBounds(10, yOffset, 360, panelHeight); // 패널 위치와 크기 설정
	        followerPanel.setBackground(new Color(255, 255, 255)); // 패널 배경 색상
	        followerPanel.setBorder(BorderFactory.createLineBorder(new Color(235, 235, 235), 2)); // 회색 테두리, 두께 2
	        ...
	        followerPanel.add(userImageButton);

	        // 유저 사진 로드
	        if (follower[2] != null && !follower[2].isEmpty()) {
	            try {
	                URI uri = new URI(follower[2]);
	                URL url = uri.toURL();
	        ...
	        containerPanel.add(followerPanel);
	...
	private java.util.List<String[]> loadFollowers(String currentUser) {
	    java.util.List<String[]> allFollowers = new ArrayList<>();

	    try (Connection conn = DriverManager.getConnection(DB_URL, DB_USER, DB_PASSWORD)) {
	        // 현재 사용자를 팔로우하는 모든 유저를 가져오는 SQL 쿼리
